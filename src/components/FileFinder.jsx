...
const FileFinder = ({tabs, selectedTab, setSelectedTab}) => {
  ...
  return (
    <div className="bg-fileFinder h-full w-72 min-w-fit border-r-1 border-gray-900 hidden md:block text-gray-500 p-1 pl-0">
      ...
      {sourceActive && (
        <div className="flex flex-col items-start mt-1">
          {tabs.map(tab =>
            <button key={tab.value} className={'flex items-center text-md pl-6 py-[2px] hover:bg-superLightGray w-full '.concat(selectedTab === tab.value ? 'bg-superLightGray text-white' : '')} onClick={() => setSelectedTab(tab.value)}>
